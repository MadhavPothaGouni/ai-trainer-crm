...
import type { PageResponse, QuoteDto, QuoteStatus } from "../../types/api";
...
export default function QuoteListPage() {
  ...
  const [result, setResult] = useState<PageResponse<QuoteDto> | null>(null);
  ...
  useEffect(() => {
    let cancelled = false;
    setIsLoading(true);
    listQuotes({ page, size: PAGE_SIZE, sort: "createdAt,desc" })
      .then((res) => {
        if (!cancelled) setResult(res);
      })
      .catch((err: unknown) => {
        if (!cancelled) setError(err instanceof ApiError ? err.message : "Could not load quotes.");
      })
      .finally(() => {
    ...
    <div className="flex flex-col gap-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-slate-500">Priced proposals tied to an opportunity.</p>
        </div>
        <Link to="/quotes/new">
