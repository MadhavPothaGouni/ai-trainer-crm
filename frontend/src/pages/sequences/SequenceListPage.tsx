...
import type { PageResponse, SequenceDto } from "../../types/api";

const PAGE_SIZE = 20;

export default function SequenceListPage() {
  const [page, setPage] = useState(0);
  const [result, setResult] = useState<PageResponse<SequenceDto> | null>(null);
  ...
  useEffect(() => {
    let cancelled = false;
    setIsLoading(true);
    listSequences({ page, size: PAGE_SIZE, sort: "name,asc" })
      .then((res) => {
        if (!cancelled) setResult(res);
      })
      .catch((err: unknown) => {
        if (!cancelled) setError(err instanceof ApiError ? err.message : "Could not load sequences.");
      })
      .finally(() => {
    ...
    <div className="flex flex-col gap-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-slate-500">Reusable outreach cadences a Lead or Contact can be enrolled in.</p>
        </div>
      ...
      <div className="overflow-hidden rounded-lg border border-slate-200 bg-white">
        <table className="w-full text-left text-sm">
          ...
          <tbody className="divide-y divide-slate-100">
            ...
            {result?.content.map((sequence) => (
              <tr key={sequence.id} className="hover:bg-slate-50">
                <td className="px-4 py-3">
                  <Link to={`/sequences/${sequence.id}`} className="font-medium text-slate-900 hover:underline">
                    {sequence.name}
                  </Link>
                </td>
                <td className="px-4 py-3 text-slate-600">{sequence.steps.length}</td>
                <td className="px-4 py-3">
                  {sequence.active ? (
