...
import type { PageResponse, ProductDto } from "../../types/api";

const PAGE_SIZE = 20;

export default function ProductListPage() {
  const [page, setPage] = useState(0);
  const [result, setResult] = useState<PageResponse<ProductDto> | null>(null);
  ...
  useEffect(() => {
    let cancelled = false;
    setIsLoading(true);
    listProducts({ page, size: PAGE_SIZE, sort: "name,asc" })
      .then((res) => {
    ...
    <div className="flex flex-col gap-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-slate-500">The shared catalog quotes are built from.</p>
        </div>
            ...
            {result?.content.map((product) => (
              <tr key={product.id} className="hover:bg-slate-50">
                <td className="px-4 py-3">
                  <Link to={`/products/${product.id}`} className="font-medium text-slate-900 hover:underline">
                    {product.name}
                  </Link>
                </td>
