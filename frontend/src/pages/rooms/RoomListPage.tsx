...
import type { PageResponse, RoomDto } from "../../types/api";

const PAGE_SIZE = 20;

export default function RoomListPage() {
  const [page, setPage] = useState(0);
  const [result, setResult] = useState<PageResponse<RoomDto> | null>(null);
  ...
  useEffect(() => {
    let cancelled = false;
    setIsLoading(true);
    listRooms({ page, size: PAGE_SIZE, sort: "label,asc" })
      .then((res) => {
        if (!cancelled) setResult(res);
      })
      .catch((err: unknown) => {
        if (!cancelled) setError(err instanceof ApiError ? err.message : "Could not load rooms.");
      })
      .finally(() => {
    ...
    <div className="flex flex-col gap-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-slate-500">The facility's bookable spaces.</p>
        </div>
        <Link to="/rooms/new">
            ...
            {result?.content.map((room) => (
              <tr key={room.id} className="hover:bg-slate-50">
                <td className="px-4 py-3">
                  <Link to={`/rooms/${room.id}`} className="font-medium text-slate-900 hover:underline">
                    {room.label}
                  </Link>
                </td>
